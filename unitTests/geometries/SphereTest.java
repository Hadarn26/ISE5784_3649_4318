...
class SphereTest {
    ...
    void testFindIntsersections() {
        ...
        assertNull(sphere.findIntsersections(new Ray(new Point(0,-0.5,0), v110.scale(-1))),
        ...
        final var result5 = sphere.findIntsersections(new Ray(new Point(1.61,0.61,0.51),
                        v310.scale(-1))).stream().sorted(Comparator
        ...
        exp=List.of(p200);
        final var result8 = sphere.findIntsersections(new Ray(Point.ZERO,v100))
        ...
        exp=List.of(p200);
        final var result9 = sphere.findIntsersections(new Ray(new Point(1.5,0,0),v100))
        ...
        final var result10 = sphere.findIntsersections(new Ray(p100,v310))
        ...
        assertNull(sphere.findIntsersections(new Ray(p200, v100)),
                "Ray starts at sphere and goes outside");

        // TC12: Ray starts after sphere (0 points)
        assertNull(sphere.findIntsersections(new Ray(p300, v100)),
                "Ray starts after sphere");

        // **** Group: Ray's line is tangent to the sphere (all tests 0 points)
        // TC13: Ray starts before the tangent point
        assertNull(sphere.findIntsersections(new Ray(new Point(0,0,1), v100)),
                "Ray starts before the tangent point");

        // TC14: Ray starts at the tangent point
        assertNull(sphere.findIntsersections(new Ray(new Point(1,0,1), v100)),
                "Ray starts at the tangent point");

        // TC15: Ray starts after the tangent point
        assertNull(sphere.findIntsersections(new Ray(new Point(3,0,1), v100)),
