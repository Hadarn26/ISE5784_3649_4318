...
class GeometriesTests {
    ...
    void testFindIntsersections() {
        ...
        List<Point> result = geos.findIntsersections(new Ray(new Point(0, 2, 0), new Vector(1, 0, 0)));
        ...
        assertNull(geos.findIntsersections(new Ray(new Point(0, 2, 0), new Vector(-1, 0, 0))),
                "There shouldn't be any intersections");

        // TC03: Ray only intersects one geometry (1 point)
        result = geos.findIntsersections(new Ray(new Point(0, 2, 0), new Vector(1, -2, 0)));
        assertEquals(1, result.size(), "There should be one intersection");

        // TC04: No geometries exist (0 points)
        geos = new Geometries();
        assertNull(geos.findIntsersections(new Ray(new Point(1, 0, 1), new Vector(1, 0, 2))),
                "There shouldn't be any intersections");
        ...
        result = geos.findIntsersections(new Ray(new Point(0, 2, 0), new Vector(1, 0, 0)));
